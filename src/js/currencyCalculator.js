import {currencies} from "./currences.js";

const curInputRef = document.getElementById('currencyInput')
const curQuantityRef = document.getElementById('currencyQuantity')
...
async function calculateCurrencies() {
    if (curInputRef.value !== '0' && curQuantityRef.value > 0 && curOutputRef.value !== '0') {
        ...
        if(curInput && curOutput) {
            resultRef.innerText = curQuantityRef.value * curInput / curOutput;
        } else {
...
curQuantityRef.addEventListener('change', calculateCurrencies)
